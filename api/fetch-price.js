...
function stripHtml(html) {
  return html
    ...
    .replace(/<!--[\s\S]*?-->/g, ' ')
    // 打ち消し線の金額（通常価格）が分かるように印を残す
    .replace(/<(del|s|strike)\b[^>]*>([\s\S]*?)<\/\1>/gi, ' 【通常価格(打ち消し線)】$2【/通常価格】 ')
    .replace(/<span[^>]*line-through[^>]*>([\s\S]*?)<\/span>/gi, ' 【通常価格(打ち消し線)】$1【/通常価格】 ')
    .replace(/<[^>]+>/g, '\n')
    .replace(/&nbsp;/g, ' ')
    .replace(/&yen;/g, '¥')
    .replace(/&amp;/g, '&')
...
【キャンペーン価格の扱い】
キャンペーン価格（期間限定の値引き）と通常価格が併記されている場合は、必ず通常価格の方を返してください。
HTML では通常価格に打ち消し線（<del>、<s>、<strike>、line-through）が付き、本文テキストでは同じマスに2つの金額が並ぶことがあります。その場合は高い方（元の価格）が通常価格です。
「キャンペーン」「SALE」「今だけ」「〇%OFF」などの表記が付いた金額は使わないでください。
金額はページの表記どおり（税込表示なら税込のまま）返し、税の換算はしないでください。
