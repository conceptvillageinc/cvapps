...
### Phase A-3: ネット印刷価格収集を新形式へ ✅ 対応不要
移植対象だった `collectWebPrices` を廃止したため、新形式へ移す機能が無くなった。
新形式で仕入価格を取り込む手段は A-1（仕入先見積の取込）が担う。
...
- [x] `collectWebPrices` → **移植せず廃止**（機能ごと削除）

#### collectWebPrices を廃止した理由
移植元のコードを読んだところ、この関数は**ネット印刷各社のサイトを一切見ていなかった**。
「一般的な市場価格帯を参考に現実的な価格を推定してください」とAIに尋ねているだけで、
返ってくるのは**実在しない推定価格**。それが「各社見積価格」の比較表に入り、
クライアントに出す見積の原価として使われていた。UIに「推定値」の注意書きはあったが、
表に4社並ぶと実際に取得した価格と見分けがつかない。

代替手段があるため、機能ごと削除した:
- `api/fetch-price.js`（実際にページを取得して抽出。今回移植済み）
- 価格マスタ（実際に収集した価格を保存してある）

`WEB_VENDORS` は残している。「各社見積価格」に手入力で行を足すときの会社名の
候補として使われており、価格は入らないため。

### Phase 4-B: メンバー招待 ✅ 完了
手順は `docs/phase4b-setup.md`。
- [x] `invitations` テーブルと、招待された人だけを登録するトリガー
- [x] `/api/invite`（管理者のみ）。**権限確認はサーバー側で行う**——
      画面でボタンを隠すだけだと、APIを直接呼べば誰でも自分を管理者にできてしまう
- [x] ユーザー管理画面に「招待中（未ログイン）」の一覧と取り消し
- [x] 新規利用者の既定権限を admin → user に変更（既存の行は変えない）
- [x] `0003_invitations.sql` の実行

### メール送信（Google Workspace）✅ 完了（本番動作確認済み）
手順は `docs/gmail-setup.md`。招待メールと見積依頼メールの両方で使う。
- 外部の配信サービスではなく、サービスアカウント＋ドメイン全体の委任で
  会社のメールボックスから送る。追加費用なし、DNS（SPF/DKIM）の設定も不要。
  印刷会社から見て実在の会社アドレスから届き、返信も通常どおり受け取れる。
- **差出人は操作した本人のアドレス。** ドメイン全体の委任で本人のメールボックスから
  送るため、送信控えが本人の Gmail「送信済み」に残り、返信も本人に直接届く。
  `GMAIL_ALWAYS_CC` で全送信の控えを1箇所に集約できる。
- **宛先も差出人も画面からは指定できない。** 宛先は会社名から印刷所マスタを引き、
  差出人はサーバーで検証済みのログイン情報を使う。リクエストの中身から決められる
  作りだと、任意の相手への送信や、社内の別の人へのなりすましができてしまう。
- 送信の成否にかかわらず email_logs に記録する（失敗は `failed`）。
  「送ったつもり」を防ぐため。
- 未設定でもアプリは壊れない。招待は登録され、URLを手で伝える運用になる。
- スコープは `gmail.send` のみ。閲覧・削除はできない。

#### これで解消した課題
`EmailLog` にレコードを作るだけで**実際のメール送信をしていなかった**問題
（Base44 時代からの状態。移行中に見つけた課題として記録していたもの）。

**招待メールは自動送信しない。** Googleログインのみの構成なので、Supabase の
招待メール（メール内リンクでログインする仕組み）とは噛み合わない。管理者が
URLを伝える運用にしている。

#### 途中で見つけて直したもの
- アダプタの `filter()` が `null` を `eq` で比較していた。SQL では `NULL = NULL` が
  真にならないため、「未対応のものだけ」のような絞り込みが常に0件になっていた。
  `is` を使うよう修正（招待一覧が空になる問題として表面化した）。
...
- [x] 全機能の本番動作確認
- [ ] 独自ドメイン設定（任意）
- [ ] 実運用で数件通したのち、Base44アプリを解約

**Base44 への依存は解消済み。** `@base44/sdk` は削除され、AI呼び出し・
ファイル保管・サーバー関数・認証・メンバー管理はすべて自前の構成で動いている。

#### 設定作業でつまずいた点（再構築時の注意）
手順書に値を貼る場所を作るときは、説明文と実際の値を同じ列に置かないこと。
「JSONの client_email」という説明文がそのまま環境変数に設定され、
`The OAuth client was not found` の調査に時間を要した。
秘密鍵は部分選択で端が欠けやすいため、JSONファイルごと貼らせる方式にした。
...
- **メール送信が実装されていない** — `EmailPreview.jsx` の「送信記録」ボタンは `EmailLog` に
  レコードを作るだけで、実際のメール送信は行っていなかった。→ Google Workspace 連携で解消済み。
