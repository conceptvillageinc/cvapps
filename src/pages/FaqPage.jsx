...
// AIに渡す CV AX KIT（CVアプリ）の機能概要（回答を実際の仕様に沿ったものにするためのコンテキスト）
const APP_CONTEXT = `
CV AX KIT（CVアプリ。旧称 CV見積アプリ）は、株式会社コンセプト・ヴィレッジ社内向けの見積作成・管理アプリです。主な機能は以下の通り。
  ...
  const handleAskAi = async () => {
    ...
    try {
      ...
      const result = await db.integrations.Core.InvokeLLM({
        prompt: `あなたは「CV AX KIT（CVアプリ）」の使い方サポートアシスタントです。以下のアプリの機能概要を参考に、メンバーからの質問に日本語で簡潔に回答してください。手順がある場合は番号付きリストで。コンテキストにない内容（アプリの仕様にない細かい仕様など）は想像で答えず、「この点は情報がないため、担当者に確認してください」と正直に伝えてください。
